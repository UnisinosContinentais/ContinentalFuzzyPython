...
class OutputService:
    ...
    def create_from_fis_block(self,
        ...
        for f_line in fis_lines:
            ...
            # arquivo .fis
            if l_attr[0] == 'Name':
                self.output.name = l_attr[1][:-1].replace("'", "")
            if l_attr[0] == 'Range':
                range_list = (l_attr[1][1:-1].split())
                self.output.range = [float(range_list[0]),
                                     float(range_list[1])]
            if l_attr[0] == 'NumMFs':
                self.output.num_mfs = int(l_attr[1])
            if l_attr[0][:2] == 'MF':
        ...
        if len(mfs_list) == self.output.num_mfs:
            for mf in mfs_list:
                ...
                if inference_type == ControllerType.mamdani:
                    ...
                    try:
                        _ = SugenoOutputFunctions[func_list[0]]
                    except Exception:
                        raise Exception(
                            f"Função de pertinência {func_list[0]}, "
                ...
                mf_service.create_membership_function(mf_function,
                                                      ...
                                                      values_list,
                                                      num_inputs)
                self.output.add_mfs(mf['num'], mf_service.membership_function)

        else:
            raise Exception("Quantidade de funções de pertinência para "
                            f"a variável {self.output.name} é diferente "
